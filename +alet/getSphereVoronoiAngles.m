...
for iCell = 1:length(cells)
    ...
    for iPoint = 1:(size(v_xyz, 1)-1)
        p = p + dpq.alet.getGreatCircleAngle(v_xyz(iPoint+1, :), v_xyz(iPoint, :));
    end
...
if( false )
    ...
    xyzTmp = dpq.coord.sph2cart(aedTmp);
    for iCell = 1:length(cells)
...
% cart to sph
xyz = unique(s.spawn, 'rows');
aed = dpq.coord.cart2sph(xyz);
n = size(xyz, 1);

% compute cell areas
[cellAngles, cellShapeIndices] = dpq.alet.getSphereVoronoiAngles(aed);
